...
function ContactRow({
  k, v, href, copyText, copied, onCopy,
}: {
  k: string
  v: string
  href?: string
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '90px minmax(0, 1fr) auto', gap: 12, padding: '10px 0', borderBottom: '1px dashed var(--line)', alignItems: 'center' }}>
      ...
      {href ? (
        <a
          ...
          rel="noreferrer"
          className="mono"
          style={{ fontSize: 13, color: 'var(--ink-2)', textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
        >
          {v}
        </a>
      ) : (
...
export function Contact() {
  ...
  return (
    <Section
      ...
      label="// 05 — contact"
      title="Let's talk."
      kicker="Open to junior / mid roles, ideally on a .NET-leaning team. Hybrid in Bulgaria or remote across EU."
    >
      <div className="contact-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.1fr) minmax(0, 1fr)', gap: 28 }}>
          ...
          <div className="card" style={{ padding: 28 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault()
                  alert('Drop a real CV at denis.tsranski@gmail.com — placeholder for the live build.')
                }}
              >
        ...
        <Reveal delay={100}>
          ...
          >
            ...
            <label className="mono" style={{ fontSize: 11, color: 'var(--ink-2)', display: 'block' }}>name</label>
            <input className="input" placeholder="who's asking?" style={{ marginTop: 4, marginBottom: 12 }} />

            <label className="mono" style={{ fontSize: 11, color: 'var(--ink-2)', display: 'block' }}>email</label>
            <input className="input" placeholder="you@company.com" type="email" style={{ marginTop: 4, marginBottom: 12 }} />

            <label className="mono" style={{ fontSize: 11, color: 'var(--ink-2)', display: 'block' }}>message</label>
            <textarea
              className="input"
              rows={5}
              placeholder="hi denis — we're hiring a junior .net dev…"
              style={{ marginTop: 4, resize: 'vertical' }}
            />

            <button className="btn btn-accent" type="submit" style={{ marginTop: 12, width: '100%', justifyContent: 'center' }}>
              {sent ? '✓ message queued (demo)' : 'send →'}
            </button>
